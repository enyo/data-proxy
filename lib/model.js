...
Model.prototype = {
  ...
  setData: function(data, errorCallback, skipValidation) {
    this.data = data;
    return (skipValidation) ? null : this.validate(errorCallback);
  },

  /**
   * Validates the data object and updates the data with sanitized values.
   * 
   * @property {Function} errorCallback Called if validation fails.
   * @api public
   */
  validate: function(errorCallback) {
    return this._validateRecursive(this.data, this.schema.definitionProperty, errorCallback || function(path, errorCode) { throw new Error("Validation error in '" + path + "': " + errorCode) }, this.constructor.modelName);
  },
  ...
  _validateRecursive: function(value, property, errorCallback, path) {
      ...
      if (property.type === Object) {
        ...
        _.each(property.child, function(childProperty, key) {
          ...
          if (childValue instanceof Property.INVALID) {
            delete value[key];
            if (errorCallback) errorCallback(childPath, Model.INVALID_VALUE);
            return; // Continue loop
          }
    ...
    // // Holds a list of provided keys to see at the end if there are undefined ones.
    // var providedKeys = Object.keys(obj);

    // _.each(property, function(schemaProperty, key) {
    //   var schemaProperty = property[key]
    //     , value = obj[key];
    //   // Remove the key
    //   providedKeys.splice(providedKeys.indexOf(key), 1);


    //   value = schemaProperty.validate(value);

    //   if (value instanceof Property.INVALID) {
    //     invalidValue(obj, key);
    //     return; // Continue loop
    //   }
    ...
    //     _.each(value, function(arrayValue, i) {
    //       value[i] = self._validateRecursive(arrayValue, schemaProperty.child, errorCallback);
    //     });
    //   }
    //   else if (schemaProperty.type === Object) {
    //     // Iterator over the properties
    //     value = self._validateRecursive(value, schemaProperty.child, errorCallback);
    //   }
