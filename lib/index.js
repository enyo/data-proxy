...
DataProxy.prototype.post = function(path, options, successCallback, errorCallback) {
  ...
  if (options.body) {
    if (options.body instanceof Model) options.body = options.body.data;
      ...
      if (res.headers['content-type'].indexOf('application/json') === 0) {
        try {
          ...
          if (options.receiveAs) {
            try {
              formattedResponse.doc = new options.receiveAs(formattedResponse.dataObject);
              formattedResponse.dataObject = formattedResponse.doc.data;
            }
            catch (err) {
